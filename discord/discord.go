		...
		if m.Author != nil {
			...
			if m.Author.ID == settings.Settings.Discord.BotAdmin {
				if strings.HasPrefix(m.Content, "!join") {
					...
					go func() { // FIXME: This shouldn't be here
						// 1. Get role name to apply
						roleName := strings.TrimSpace(m.Content[8:])
						// 2. Locate roleID on server
						if guild, err := s.Guild(guildID); err == nil {
							...
							if len(roleID) > 0 {
								// 3. We found a role! Let's go through members and apply it
